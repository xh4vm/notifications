...
def _make_request_feedbacks_forgotten_bookmarks(url, method, params, model):
    return ResultResponse(
        status=HTTPStatus.OK.value,
        body=[
            model(user_id=uuid_str(), films=[uuid_str(), uuid_str()]),
            model(user_id=uuid_str(), films=[uuid_str(), uuid_str()]),
            model(user_id=uuid_str(), films=[uuid_str(), uuid_str()]),
        ],
    )
...
def _make_request_content_new_movies_for_period(url, method, params, model):
    return ResultResponse(
        status=HTTPStatus.OK.value,
        body=model(period_days=params['data']['days'], films=[uuid_str(), uuid_str(), uuid_str(), uuid_str()]),
    )
...
def _make_request_content_get_film_name(url, method, params, model):
    return ResultResponse(
        status=HTTPStatus.OK.value,
        body=model(
            film_id=params['data']['film_id'], film_name='Fake Film Name {0}'.format(params['data']['film_id'])
        ),
    )
